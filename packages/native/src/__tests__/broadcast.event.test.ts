// @ts-nocheck
/**
 * FastEvent（主类）broadcast（发布端前缀广播）单元测试
 *
 * 覆盖与 FastLiteEvent 对齐的语义,以及主类特有的 transform 配合:
 *   - emit 的 broadcast 选项(true 默认改写 / 函数改写 / null 跳过)
 *   - broadcast() 快捷方法
 *   - transform 逐最终 message（先 broadcast 后 transform,后代感知自身 type）
 *
 * 设计共识见 CONTEXT.md。
 */
...
describe("FastEvent emit broadcast 选项", () => {
    ...
    test("broadcast:true 时后代监听器收到改写为自身路径的 type", () => {
        ...
        expect(a).toHaveBeenCalledTimes(1);
        expect(a.mock.calls[0][0]).toMatchObject({ type: "a", payload: 1 });
        expect(ab).toHaveBeenCalledTimes(1);
        expect(ab.mock.calls[0][0]).toMatchObject({ type: "a/b", payload: 1 });
        expect(abc).toHaveBeenCalledTimes(1);
        expect(abc.mock.calls[0][0]).toMatchObject({ type: "a/b/c", payload: 1 });
    });
    ...
    test("broadcast:true 时 payload 原样透传给后代", () => {
        ...
        expect(ab.mock.calls[0][0]).toMatchObject({ type: "a/b", payload: { x: 1 } });
    });
    ...
    test("broadcast 函数可改写 payload 与 args", () => {
        ...
        expect(ab.mock.calls[0][0]).toMatchObject({ type: "a/b", payload: 999 });
    ...
    test("broadcast 函数仅返回 message 时 args 沿用原值", () => {
        ...
        emitter.on("a/b", ab);
        emitter.emit("a", 1, {
            broadcast: (type) => ({ type, payload: 999 }),
        });
        expect(ab.mock.calls[0][0]).toMatchObject({ type: "a/b", payload: 999 });
    ...
    test("transform 逐最终 message 执行,能感知后代 type", () => {
        const emitter = new FastEvent({
            transform: (message) => message.type,
        });
        const a = vi.fn();
        const ab = vi.fn();
        emitter.on("a", a);
...
describe("FastEvent broadcast() 快捷方法", () => {
    ...
    test("传入 callback 作为自定义改写", () => {
        ...
        const ab = vi.fn();
        emitter.on("a/b", ab);
        emitter.broadcast("a", 1, (type) => ({ type, payload: 999 }));
        expect(ab.mock.calls[0][0]).toMatchObject({ type: "a/b", payload: 999 });
    });
