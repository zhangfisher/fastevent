...
describe('类型系统测试', () => {
    ...
    describe('FastEvent基础类型检查', () => {
        ...
        test('onAny方法应正确处理通配符事件类型', () => {
            ...
            emitter.onAny<number>((message) => {
                type cases = [Expect<Equal<typeof message.type, string>>, Expect<Equal<typeof message.payload, number>>];
            });
        });
    ...
    describe('作用域类型系统', () => {
        ...
        test('作用域应正确继承和扩展元数据类型', () => {
            ...
            type metaCases = [Expect<Equal<typeof scope.options.meta, FastEventScopeMeta & FastEventMeta & CustomMeta>>];
        });
        ...
        test('作用域的once方法应正确推导事件类型', () => {
            scope.once('a', (message) => {
                type cases = [Expect<Equal<typeof message.type, 'a'>>, Expect<Equal<typeof message.payload, 1>>];
            });
            scope.once('b', (message) => {
                type cases = [Expect<Equal<typeof message.type, 'b'>>, Expect<Equal<typeof message.payload, 2>>];
            });
            scope.once('c', (message) => {
                type cases = [Expect<Equal<typeof message.type, 'c'>>, Expect<Equal<typeof message.payload, 3>>];
            });
        });
...
describe('作用域事件类型检查', () => {
    ...
    test('Scope自定义事件类型与全局事件类型合并', () => {
        ...
        // 仅限scope事件
        scope.on('m', (message) => {
            type cases = [Expect<Equal<typeof message.type, 'm'>>, Expect<Equal<typeof message.payload, number>>];
        });
        scope.once('n', (message) => {
            type cases = [Expect<Equal<typeof message.type, 'n'>>, Expect<Equal<typeof message.payload, boolean>>];
        });
        scope.onAny((message) => {
            type cases = [Expect<Equal<typeof message.type, string>>, Expect<Equal<typeof message.payload, any>>];
        });
    ...
    test('嵌套Scope合并元数据', () => {
        ...
        type cases = [
            Expect<Equal<typeof rootScope.types.events, ScopeEvents<RootEvents, 'root'> & RootScopeEvents>>,
            Expect<Equal<typeof childrenScope.types.events, ScopeEvents<ScopeEvents<RootEvents, 'root'> & RootScopeEvents, 'children'> & ChildScopeEvents>>,
            Expect<
                Equal<
                    typeof GrandsondScope.types.events,
                    ScopeEvents<ScopeEvents<ScopeEvents<RootEvents, 'root'> & RootScopeEvents, 'children'> & ChildScopeEvents, 'grandsond'> & GrandsondDScopeEvents
                >
            >,
        ];
        ...
        childrenScope.on('x', (message) => {
            type cases = [Expect<Equal<typeof message.type, 'x'>>, Expect<Equal<typeof message.payload, 200>>];
        });
        childrenScope.on('children', (message) => {
            type cases = [Expect<Equal<typeof message.type, 'children'>>, Expect<Equal<typeof message.payload, 2>>];
        });
        childrenScope.on('grandsond/x', (message) => {
            type cases = [Expect<Equal<typeof message.type, 'grandsond/x'>>, Expect<Equal<typeof message.payload, 300>>];
        });
        ...
        GrandsondScope.on('grandson', (message) => {
            type cases = [Expect<Equal<typeof message.type, 'grandson'>>, Expect<Equal<typeof message.payload, 3>>];
        });
        GrandsondScope.on('x', (message) => {
            type cases = [Expect<Equal<typeof message.type, 'x'>>, Expect<Equal<typeof message.payload, 300>>];
        });
        GrandsondScope.on('xxxx', (message) => {
            type cases = [Expect<Equal<typeof message.type, 'xxxx'>>, Expect<Equal<typeof message.payload, any>>];
        });
