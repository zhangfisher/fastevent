...
describe("WildcardPriority", () => {
    describe("SplitPath - 路径分割基础测试", () => {
        test("正确处理没有 / 的情况", () => {
            ...
            type cases = [
                // 单个段（没有 /）
                Expect<Equal<GetFixedPartCount<"click">, 1>>,
                Expect<Equal<GetFixedPartCount<"event">, 1>>,
                ...
                // 带有 / 的多段路径
                Expect<Equal<GetFixedPartCount<"a/b">, 2>>,
                Expect<Equal<GetFixedPartCount<"a/b/c">, 3>>,
            ];
        });
    });
    ...
    describe("CountFixedSegments - 计算固定段数量", () => {
        test("半通配符有固定段", () => {
            type cases = [
                Expect<Equal<GetFixedPartCount<"rooms/*/$join">, 2>>,
                Expect<Equal<GetFixedPartCount<"rooms/*/$leave">, 2>>,
                Expect<Equal<GetFixedPartCount<"api/*/users">, 2>>,
                // data/v*/detail 中的 v* 不是独立的通配符段，所以是固定段
                Expect<Equal<GetFixedPartCount<"data/v*/detail">, 3>>,
        ...
        test("精确路径全是固定段", () => {
            type cases = [
                Expect<Equal<GetFixedPartCount<"user/login">, 2>>,
                Expect<Equal<GetFixedPartCount<"click">, 1>>,
                Expect<Equal<GetFixedPartCount<"rooms/lobby/join">, 3>>,
                Expect<Equal<GetFixedPartCount<"api/v1/users">, 3>>,
            ];
        });
        ...
        test("复杂路径混合通配符", () => {
            type cases = [
                Expect<Equal<GetFixedPartCount<"a/*/b/*/c">, 3>>,
                Expect<Equal<GetFixedPartCount<"x/*/y/*/z/*">, 3>>,
                Expect<Equal<GetFixedPartCount<"*/a/*">, 1>>,
            ];
        });
    ...
    describe("优先级计算综合测试", () => {
        test("rooms/*/$join vs rooms/*/*", () => {
            ...
            type JoinSegments = GetFixedPartCount<"rooms/*/$join">;
            type AllSegments = GetFixedPartCount<"rooms/*/*">;
        ...
        test("不同层级通配符的固定段数", () => {
            type cases = [
                ...
                Expect<Equal<GetFixedPartCount<"a/*">, 1>>,
                Expect<Equal<GetFixedPartCount<"a/*/*">, 1>>,
                Expect<Equal<GetFixedPartCount<"a/*/$x">, 2>>,
                Expect<Equal<GetFixedPartCount<"a/$x/*">, 2>>,
                Expect<Equal<GetFixedPartCount<"a/$x/$y">, 3>>,
            ];
        });
