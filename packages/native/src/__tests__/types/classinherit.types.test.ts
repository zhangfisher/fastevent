...
describe('继承FastEvent类型系统', () => {
    test('默认继承方式', () => {
        ...
        myemitter1.on('test', function (this, message) {
            type cases = [Expect<Equal<typeof this, MyEvent>>, Expect<Equal<typeof message.type, 'test'>>];
        });
    ...
    test('类继承', () => {
        ...
        const emitter = new MyEvent();
        emitter.on('test', function (this, message) {
            type cases = [Expect<Equal<typeof this, MyEvent>>];
        });
        type cases = [Expect<Equal<typeof emitter.options, MyEventOptions>>];
    });
    test('类继承2', () => {
        type Events = {
            a: number;
        ...
        class MyEvent<E extends Record<string, any> = {}> extends FastEvent<Events & E> {
            test(this: FastEvent<Events>) {
                this.emit('a', 1);
        ...
        const emitter = new MyEvent();
        emitter.on('a', function (this, message) {
            type cases = [Expect<Equal<typeof message.type, 'a'>>, Expect<Equal<typeof message.payload, number>>];
        });
    });
    test('继承时提供泛型参数', () => {
        ...
        class MyEvent<E extends Record<string, any> = Record<string, any>, M extends Record<string, any> = Record<string, any>, C = never> extends FastEvent<E, M, C> {
    ...
    test('继承时重载覆盖Options', () => {
        ...
        scope1.on('test', function (this, message) {
            type cases = [Expect<Equal<typeof this, MyScope>>, Expect<Equal<typeof message.type, 'test'>>];
        });
    });
});
...
describe('继承FastEventScope类型系统', () => {
    ...
    test('继承Scope类', () => {
        ...
        class MyScope<E extends Record<string, any> = MyScopeEvents, M extends Record<string, any> = Record<string, any>, C = never> extends FastEventScope<E, M, C> {
