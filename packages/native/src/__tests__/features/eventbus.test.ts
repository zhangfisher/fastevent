...
describe('FastEventBus', () => {
    ...
    describe('节点的加入和离开', () => {
        test('节点的加入事件', () => {
            ...
            expect(nodes).toEqual(['node1', 'node2', 'node3', 'node4', 'node5']);
        });
        test('节点的离开事件', () => {
            const nodes: string[] = [];
    ...
    describe('广播消息', () => {
        ...
        test('接收所有retain总线广播指定事件', () => {
            ...
            expect(results.map((msg) => msg.type)).toEqual([`@/order/new`, `@/order/cancel`, `@/order/submit`, `@/order/pay`]);
            expect(results.map((msg) => msg.payload)).toEqual([1, 2, 3, 4]);
        });
    });
    describe('触发事件', () => {
        test('在其他节点触发事件', () => {
