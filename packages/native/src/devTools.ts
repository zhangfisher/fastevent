...
export class FlexEventDevTools {
    ...
    add(instance: FastEvent) {
        ...
        instance.options.onAfterExecuteListener = (message, returns, listeners) => {
            ...
            const sresults = listeners
                .reduce<FastEventListenerMeta[]>((results, cur) => {
                    results.push(...cur.__listeners);
                    return results;
                }, [])
                .map(
                    (listener, i) =>
                        `${listener[0].name || "anonymous"}(${listener[2]}) -> ${results[i]} ${listener[3] ? `(tag=${listener[3]})` : ""}`,
                );

            console.log(
                `FastEvent<\x1B[31m${message.type}\x1B[30m> is emit, listeners:`,
                listeners,
            );
            this.reduxStore.dispatch({
                type: message.type,
    ...
    private install() {
        ...
        console.info(
            "%c FlexEventDevTools installed. Please open <Redux devtools> to view. %c",
            "color:red;",
            "",
        );
    }
...
export function install() {
    // @ts-ignore
    if (!globalThis.__FLEXEVENT_DEVTOOLS__)
        globalThis.__FLEXEVENT_DEVTOOLS__ = new FlexEventDevTools();
}
